...
- `reconstruct_weight_torch(t, seed, device=…)`：同数学的 torch/CUDA 路径（列分块 gather/FWHT）；diag inject 在模型已在 GPU 时自动选用。`diag_gemma4_chat` 默认跳过 vocab/PLE/`lm_head`（`--inject-embeddings` 可开）。
